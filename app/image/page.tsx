...
import { isInPhotoshopPlugin, capturePSLayer, sendToPhotoshop } from '@/lib/psPlugin';
// 上传图片到img.scdn.io图床
...
// 检测是否在PS插件环境中
useEffect(() => {
  setIsPSPlugin(isInPhotoshopPlugin());
}, []);
...
// PS插件：检测是否在PS插件环境中
const [isPSPlugin, setIsPSPlugin] = useState(false);
const [capturingLayer, setCapturingLayer] = useState(false);
const [sendingToPS, setSendingToPS] = useState<string | null>(null);

// PS插件：上传PS图层为参考图
const handleUploadPSLayer = async () => {
  ...
  try {
    const result = await capturePSLayer();
    if (result.success && result.imageData) {
      // 将base64转换为File对象
      const response = await fetch(result.imageData);
      const blob = await response.blob();
      const file = new File([blob], `ps-layer-${result.layerName || 'layer'}.png`, { type: 'image/png' });
    ...
    alert('上传PS图层失败: ' + (error.message || error));
  } finally {
...
// PS插件：发送图片到Photoshop
const handleSendToPS = async (imageUrl: string) => {
  setSendingToPS(imageUrl);
  try {
    const result = await sendToPhotoshop(imageUrl, { asSmartObject: true });
    if (result.success) {
      alert('已发送到Photoshop');
    }
  } catch (error: any) {
    alert('发送到Photoshop失败: ' + (error.message || error));
  } finally {
    setSendingToPS(null);
  }
};
    ...
    {activeTask.result ? (
      <>
          ...
          {activeTask.result.map((url, index) => (
            <div key={index} className="relative group">
              ...
              <div className="absolute bottom-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                {isPSPlugin && (
                  <button
                    onClick={() => handleSendToPS(url)}
                    disabled={sendingToPS === url}
                    className="bg-blue-500 text-white px-3 py-1 rounded-lg hover:bg-blue-600 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {sendingToPS === url ? '发送中...' : '发送到PS'}
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="absolute bottom-4 right-4 flex gap-2">
          ...
          className="bg-yellow-500 text-white px-4 py-2 rounded-xl hover:bg-yellow-600 flex items-center gap-2 transition-colors shadow-sm"
          >
          ...
          {isPSPlugin && (
            <button
              onClick={() => {
                if (activeTask.result && activeTask.result.length > 0) {
                  activeTask.result.forEach((url, i) => {
                    setTimeout(() => handleSendToPS(url), i * 500);
                  });
                }
              }}
              disabled={!!sendingToPS}
              className="bg-blue-500 text-white px-4 py-2 rounded-xl hover:bg-blue-600 flex items-center gap-2 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
{/* PS插件：上传PS图层按钮，仅在PS插件环境中显示 */}
{isPSPlugin && referenceImages.length < 5 && (
  <button
    type="button"
    onClick={handleUploadPSLayer}
    disabled={capturingLayer || uploading}
    className="w-full h-12 mb-4 bg-blue-50 text-blue-600 border-2 border-blue-200 rounded-lg hover:bg-blue-100 hover:border-blue-300 transition-colors flex items-center justify-center gap-2 text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
  >
    {capturingLayer ? (
      <>
